...
module.exports = (function () {
  ...
  args = [
    ...
    ['e', 'extension', 'sets the output file extension',                   '.html'],
  ...
  function interpret(argv) {
    ...
    for (i = 0; i < files.length; i++) {
      ...
      if (opts.tokens) {
        ...
        if (opts.verbose) stdout(readables.join('\n'));
        else stdout(readables.join(' '));
        process.exit(0);
      }
      ...
      if (opts.print) {
        ...
        fs.writeFileSync(path.resolve(path.resolve(path.dirname(files[i]), opts.output), /(.+)(\.\w+)+?/.exec(path.basename(files[i]))[1] + '.' + opts.extension.replace(/^\./,'')), result);
      }
    }
  }
