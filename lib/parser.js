...
module.exports = (function () {
  ...
  /**
   * Finds a `%` command token and executes.
   * NOTE: This is a mess and would require a complete rebuild of the parser
   * to make it beautiful (read sense). This is more of a syntax test rather
   * than the truth.
   * @return {Boolean} success
   */
  function commandToken() {

    if (!COMMAND.test(token[0])) {
      return false;
    }

    var nextToken    = null;
    var origin       = token;
    var originIndex  = needle;
    var endIndex     = 0;
    var originIndent = indent;

    // %for x from y to|through z ==============================================
    if (token[1] === 'for') {

      var iterator, start, end;

      // find the iterator variable name ---------------------------------------
      nextToken = peekAhead();
      if (!VARIABLE.test(nextToken[0])) {
        throwExpection('VARIABLE', nextToken);
      }
      nextToken = peekAhead(2);
      ...
      if (!IDENTIFIER.test(token[0]) && token[1] !== 'from') {
        throwExpection('IDENTIFIER', token);
      }
      next();

      // find the start value --------------------------------------------------
      if (!NUMBER.test(token[0])) {
        throwExpection('NUMBER or VARIABLE', token);
      }
      if (VARIABLE.test(token[0])) {
        next();
        start = getVariableValue(token[1], host);
        if (!start) {
          throwErrorWithReference('Variable "' + token[1] + '" is `undefined`.', token);
        }
      } else {
        start = token[1];
      }
      ...
      if (!(/to|through/i).test(token[1])) {
        throwErrorWithReference('For-loops with range require `from x to|through y` syntax.', token);
      }
      next();
      ...
      if (VARIABLE.test(token[0])) {
        next();
        end = getVariableValue(token[1], host);
        if (!end) {
          throwErrorWithReference('Variable "' + token[1] + '" is `undefined`.', token);
        }
      } else {
      ...
      // find loop child tokens and correct dentation --------------------------
      var childTokens = [];
      var loopIndent = originIndent + 1;
      while(loopIndent > originIndent) {
        next();
        if (BOF_EOF.test(token[0])) {
          break;
        }
        if (END_OF_LINE.test(token[0]) && !DENTATION.test(peekAhead()[0])) {
          break;
        }
        if (DENTATION.test(token[0])) {
          loopIndent += token[1];
      ...
      indentBy(childTokens, -1);

      // remove loop and children from token stream ----------------------------
      input.splice(originIndex, endIndex - originIndex + childTokens.length + 1);
      needle = originIndex;

      // append iterator variable ----------------------------------------------
      host.variables[iterator] = start;

      // inject child tokens n-times and increase iterator ---------------------
      for (var i = start; i <= end; i++) {
        var clonedChildren = childTokens.slice();
        clonedChildren.unshift(['VARIABLE', '$'], ['IDENTIFIER', iterator],['ASSIGN','='],['NUMBER', i]);
        input = tools.mergeArrayAt(input, clonedChildren, needle);
        needle += clonedChildren.length;
      }

      // reset needle and end --------------------------------------------------
      needle = originIndex;
      return true;
    }
  ...
  function fillVariables(value, scope, ignoreMixinScope) {
    ...
    if (/\$[a-z0-9-_]/gi.test(value)) {
      ...
      while ((index = value.slice(index + 1,-1).indexOf('$')) > -1) {
        ...
        match = /\$([a-z0-9-_]+)/i.exec(value.slice(index));
  ...
  function indentBy(tokens, distance) {
    var x = tokens.length;
    while (--x >= 0) {
      if (DENTATION.test(tokens[x][0])) {
        var newDepth = tokens[x][1] + distance;
        tokens[x][1] = Math.max(0, newDepth);
      }
    }
  }
  ...
  function throwErrorWithReference(message, token) {
    if (token[4] != null) {
      var rawFile = fs.readFileSync(token[4], 'utf8');
      var lines = rawFile.split('\n');
      var whitepsaceChars = '';
      var charIndex = token[3];
      while (--charIndex) {
        whitepsaceChars += ' ';
      }
      var snapshot = '\n' + lines[token[2]-1] + '\n' + whitepsaceChars + '^---------------\n\n';

      throwError(snapshot + message + ' at line: ' + token[2] + ':' + token[3] + ' in ' + token[4]);
    } else {
