...
module.exports = (function () {
    ...
    if (ATTRIBUTE.test(token[0])) {
      ...
      if (IDENTIFIER.test(nextToken[0])) {
        ...
        if (ASSIGN.test(nextToken[0])) {
          ...
          if (STRING_NUMBER.test(nextToken[0])) {
          ...
          } else throwExpection('STRING, NUMBER or VARIABLE', nextToken);
  ...
  function callToken() {
    ...
    var nextToken, mixin, target, parameters;
    ...
    if (CALL.test(token[0])) {
      ...
      if (PARENTHESIS_START.test(nextToken[0])) {
        ...
        while(PARENTHESIS_END.test(next()[0]) == false) {
          ...
          if (SEPARATOR.test(token[0])) continue;
          if (STRING_NUMBER.test(token[0])) {

              parameters.push(token[1]);

          } else throwExpection('STRING or NUMBER', token);
  ...
  function markupToken() {
    ...
    if (MARKUP.test(token[0])) {
      host.children.push(new Block({value: token[1]}));
    ...
    function parseChild(source) {
      ...
      mixin.params.forEach(function (param) {
        ...
        for (var attribute in source.attributes) {
          source.attributes[attribute] = source.attributes[attribute].replace('$' + param, parameters[index]);
        }

        if (source.id && typeof source.id === 'string')
          source.id = source.id.replace('$' + param, parameters[index]);
        if (source.name && typeof source.name === 'string')
          source.name = source.name.replace('$' + param, parameters[index]);
        if (source.value && typeof source.value === 'string')
          source.value = source.value.replace('$' + param, parameters[index]);
