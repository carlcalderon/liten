...
module.exports = (function () {
  ...
  function commandToken() {
    ...
    if (token[1] === 'for') {
      ...
      // end
      if (!token[1] === 'to') {
        throwErrorWithReference('For-loops with range require `from x to y` syntax.', token);
      }
      ...
      // Loop though children x to y times and push to current || host

    }

    // var command = {};
    // command.tag = new Block({
    //   type: "command",
    //   name: origin[1],
    //   parent: host
    // });
    // (current || host).children.push(command);
